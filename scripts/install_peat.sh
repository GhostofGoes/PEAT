...
# man page to /usr/local/share/man/man1, updates the
# manual database (if 'mandb' command is present), and
# installs tab completions for bash, zsh, and fish.
...
# Tab completions are generated by the installed binary, so they always
# match its version. Failures here don't fail the install.
install_completion() {
  shell=$1
  dst=$2
  if mkdir -p "$(dirname "$dst")" && "$BIN_DST" completion "$shell" > "$dst"; then
    chmod 0644 "$dst"
  else
    rm -f "$dst"
    say "WARN: failed to install $shell tab completion"
  fi
}

say "Installing tab completions for bash, zsh, and fish..."
